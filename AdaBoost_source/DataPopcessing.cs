...
namespace DataProcessing
{
    ...
    class TOOL
    {
        public void MakeSplitData(string before_file_path)
        {
            ...
            else
            {
                ...
                SaveToCsv("splitTrainData.csv", after_train_data); // トレーニングデータをCSVファイルに保存
                SaveToCsv("splitTestData.csv", after_test_data);  // テストデータをCSVファイルに保存
            }
        }
    ...
    class CLOSSVALIDATION
    {
        ...
        public double average_score { get; private set; }  //正答率の平均 プロパティとして保持して別クラスで使用可能にする
